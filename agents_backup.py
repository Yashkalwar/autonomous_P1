...
class DeliberationCore:
    ...
    def __init__(self, llm_client=None):
        ...
        and create content that is clear, professional, and actionable."""
            ...
            # Enhanced parameter extraction for email tasks
            if primary_tool == ToolType.GMAIL:
                parameters = self._enhance_email_parameters(query, parameters)
    ...
    def _enhance_email_parameters(self, query: str, existing_params: dict) -> dict:
        """Enhance email parameters with document references and content requirements."""
        import re
        
        enhanced_params = existing_params.copy()
        query_lower = query.lower()
        
        # Detect document references
        if "jp.txt" in query_lower or "document" in query_lower:
            enhanced_params["document_reference"] = "JP.txt"
        
        # Detect content requirements and formatting requests
        content_requirements = []
        
        # Check for bullet points with numbers
        bullet_match = re.search(r'(\d+)\s*(?:bullet\s*)?points?', query_lower)
        if bullet_match or any(word in query_lower for word in ["bullet", "points", "list"]):
            if bullet_match:
        ...
        elif any(word in query_lower for word in ["summary", "summarize", "summarise"]):
            content_requirements.append("summary")
        
        # Check for brief/concise requests
        ...
        if content_requirements:
            enhanced_params["content_requirement"] = " ".join(content_requirements)
...
class DeliberationCore:
    ...
    def _generate_email_content(self, plan: Plan) -> Dict[str, Any]:
        """Generate email content using LLM with intelligent content processing."""
        ...
        content_requirement = step.parameters.get("content_requirement") or step.parameters.get("content")
        document_reference = step.parameters.get("document_reference")
        ...
        try:
            # Process document content if referenced
            processed_content = None
            if document_reference and content_requirement:
                processed_content = self._process_document_content(document_reference, content_requirement, plan.user_query)
            
            # Generate email using LLM with full context
            if processed_content:
                llm_content = {
                    "subject": subject_hint or self._generate_subject_from_content(processed_content, plan.user_query),
                ...
                llm_content = self.llm.generate_email_content(
                    ...
                    summary_text=content_requirement,
                )
    ...
    def _process_document_content(self, document_reference: str, content_requirement: str, user_query: str) -> str:
        """Process document content based on user requirements using LLM."""
        try:
            # Load document content
            from pathlib import Path
            documents_dir = Path("user_documents")
            doc_path = documents_dir / document_reference
            
            if not doc_path.exists():
                return f"Document {document_reference} not found."
            
            raw_content = doc_path.read_text(encoding='utf-8').strip()
            request_lower = content_requirement.lower()
            
            # Determine processing type based on user requirement
            if any(word in request_lower for word in ["bullet", "points", "list"]):
                return self._generate_bullet_points(raw_content, content_requirement)
            elif any(word in request_lower for word in ["summary", "summarize", "summarise"]):
                return self._generate_summary(raw_content)
            elif any(word in request_lower for word in ["brief", "short", "concise"]):
                return self._generate_brief_content(raw_content)
            elif any(word in request_lower for word in ["highlight", "key", "important", "main"]):
                return self._generate_highlights(raw_content)
            elif any(word in request_lower for word in ["overview", "intro", "introduction"]):
                return self._generate_overview(raw_content)
            else:
    ...
    def _generate_bullet_points(self, raw_content: str, requirement: str) -> str:
        """Generate bullet points from content."""
        import re
        # Extract number if specified
        number_match = re.search(r'(\d+)\s*(?:bullet\s*)?points?', requirement.lower())
...
Key Points:"""
    ...
    def _generate_summary(self, raw_content: str) -> str:
...
Summary:"""
    ...
    def _generate_brief_content(self, raw_content: str) -> str:
...
Brief version:"""
    ...
    def _generate_highlights(self, raw_content: str) -> str:
        """Generate key highlights."""
        prompt = f"""Please extract the key highlights from the following content for an email:

{raw_content}

Make it:
- Professional and engaging
- Focus on the most important achievements and skills
- 3-4 key highlights
- Suitable for email communication
...
Key Highlights:"""
    ...
    def _generate_overview(self, raw_content: str) -> str:
        """Generate overview/introduction."""
        prompt = f"""Please create a professional overview/introduction from the following content for an email:
...
Overview:"""
    ...
    def _generate_subject_from_content(self, content: str, user_query: str) -> str:
        """Generate appropriate subject line from content."""
        prompt = f"""Based on the following email content and original request, suggest a professional email subject line:

Original request: {user_query}
Email content: {content[:200]}...
